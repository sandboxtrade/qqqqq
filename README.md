# Virtual Companion / Yuzuki v0.17.0

Mobile-first virtual companion with a persistent local Character Brain, memory, relationship/emotion state, world simulation, Firebase synchronization and a GPT-first conversation layer.

From v0.17.0, normal conversation is generated by OpenAI through the authenticated Cloudflare Worker. Local Brain remains authoritative for durable memory, personality, emotion/relationship state, world state, romance/intimacy boundaries and hard character constraints. The local dialogue renderer remains a resilient fallback when cloud generation is unavailable.

## Current compact structure

```text
src/
├─ ai/                 # GPT-first Cloudflare conversation transport + legacy adapters
...
`USER -> Local state/memory retrieval + NLU -> Character Brain state/constraints -> compact conversation packet -> Cloudflare/OpenAI GPT conversation -> invariant guard -> event/persistence pipeline -> Firestore/live sync -> UI`

If the cloud path fails, `LocalDialogueRenderer -> local guard` supplies the fallback reply for the same turn. GPT never writes Firestore or mutates durable character state directly.
...
- GPT-first conversation client: `src/ai/cloud-language.ts`
...
The local fallback can still operate without OpenAI, but production-quality normal dialogue expects the deployed Cloudflare Worker with its server-side `OPENAI_API_KEY` secret. The key never belongs in the browser bundle.
...
- `tests/cloud-language.mjs` — GPT-first Cloudflare/Auth/App Check/telemetry contract.
- `tests/local-dialogue.mjs` — Local fallback/NLU coverage and stress scenarios.
