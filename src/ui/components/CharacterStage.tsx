      ...
      <div className="stage-bottom">
        <div className="character-title">
          <div>
            ...
            <p>
              {moodLabel} · {" "}
              {timeLabel[runtime?.world.timeOfDay ?? ""] ?? "сейчас"}
            </p>
          </div>
          <span className="relationship-badge">
            {relationshipLabel[runtime?.relationship.stage ?? "new"] ??
              "знакомство"}
          </span>
        </div>
        <div className="quick-actions">
          <button type="button" onClick={() => onNavigate("together")}>
            <Icon name="together" size={18} />
            <span>Вместе</span>
          </button>
          <button type="button" onClick={() => onNavigate("look")}>
