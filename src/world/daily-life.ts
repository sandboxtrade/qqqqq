...
import type { WorldActivity, WorldEventSnapshot, WorldLocation } from './world-types';
...
const templates: EventTemplate[] = [
  {
    ...
    summary: 'She got absorbed in something she was reading and kept thinking about one idea afterward.',
  ...
  },
  {
    ...
    summary: 'She made satisfying progress on a personal project and felt quietly pleased with herself.',
  ...
  },
  {
    ...
    summary: 'A small everyday inconvenience irritated her for a while, though it was not serious.',
  ...
  },
  {
    kind: 'outing',
    activities: ['walk'],
    summary: 'She took a short walk to clear her head and came back a little calmer.',
  ...
  },
  {
    ...
    location: 'cafe',
    summary: 'She spent a little time at a café for a change of scenery and enjoyed the quiet break.',
...
function chooseTemplate(activity: WorldActivity, timestamp: number) {
  const candidates = templates.filter((template) => template.activities.includes(activity));
  if (!candidates.length) return null;
  return candidates[Math.floor(deterministic01(timestamp, activity) * candidates.length) % candidates.length];
}

export function maybeCreateWorldEvent(timestamp: number): WorldEventSnapshot | null {
  const routine = resolveRoutine(timestamp);
  if (!routine.isAwake || ['waking_up', 'breakfast'].includes(routine.activity)) return null;
